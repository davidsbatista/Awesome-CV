...
%
		% --- 1. Maintainership -------------------------------------------------
		\item {Core maintainer of Haystack, an open-source LLM orchestration framework used by thousands of developers globally. Sustained day-to-day operations: contributing with new components and features, managing release cycles, reviewing pull requests, triaging GitHub issues, mentoring external contributors. Together with the other team members, helped plan and led several aspects of the framework: which new components to add, public interfaces, and testing standards}
%
		% --- 2. Components, public interfaces & retrieval experiments ----------
		\item {Designed the Protocols defining Haystack's embedder and retriever interfaces, and contributed components against them: sentence-window and auto-merging retrievers, hybrid-retrievals, query expansion and multi-query retrievers, recursive and embedding-based document splitters, and LLM-based metadata and content extractors. Ran experiments comparing retrieval techniques (Hypothetical Document Embeddings, Document Summary Index, maximum marginal relevance), presented at PyCon Lithuania 2025.}
%
...
%
		...
		\item {Standardised the API across the document store backends, uniformising their operation signatures and enforcing a shared conformance test suite replacing duplicated per-backend tests with a single mixin.}
%
